...
import { Dialog, DialogTitle, DialogActions, TextField, LinearProgress } from '@mui/material';
...
import { deleteAlbumRequest, getAlbumRequest, updateAlbumRequest } from '../API/AlbumRequests';
...
export default function AlbumPage() {
    ...
    const fetchAlbum = () => {
        getAlbumRequest(id!).then(function (response) {
    ...
    const handleSubmitDelete = () => {
        deleteAlbumRequest(album!._id.$oid).then(function (response) {
            if (response.status >= 400 && response.status < 600) {
                enqueueSnackbar("Sorry, internal error occured", {
                    variant: 'error', anchorOrigin: {
                        vertical: 'top',
    ...
    const handleSubmitEdit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        updateAlbumRequest(album!._id.$oid, name, date, image_url).then(function (response) {
            if (response.status >= 400 && response.status < 600) {
                enqueueSnackbar("Sorry, internal error occured", {
                    variant: 'error', anchorOrigin: {
                        vertical: 'top',
    ...
    return (
        <>
            {albumExists ?
                        ...
                        <Grid item xs={12} sm={6} md={4}>
                            ...
                            >
                                ...
                                <CardContent sx={{ flexGrow: 1 }}>
                                    ...
                                    <Typography>
                                        {GetDateString(new Date(album!.date))}
                                    </Typography>
                                </CardContent>
                                <CardActions>
                ...
                :
                <Typography variant='h1'>Album not found</Typography>
            }
        </>
    );
}